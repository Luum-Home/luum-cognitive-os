...
``cos_lib.openai_compatible_agent_loop.run_agent`` with:

  - Model-hint → provider cascade resolution via ``cos_lib.dispatch`` conventions.
...
Usage::
    ...
    from cos_lib.agent_runner import spawn
...
from cos_lib.tool_result_envelope import wrap_if_large, ENVELOPE_THRESHOLD
    ...
    *,
    ...
    Args:
        ...
        project_dir: Root directory for event/transcript files.
            Defaults to :func:`cos_lib.paths.runtime_project_root_or_cwd`.
    ...
    if run_fn is None:
        from cos_lib.openai_compatible_agent_loop import run_agent as run_fn  # type: ignore[assignment]
