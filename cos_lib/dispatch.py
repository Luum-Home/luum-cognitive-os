...
from cos_lib.execution_profile import provider_cascade_for_profile, resolve_runtime_execution_profile
from cos_lib.paths import runtime_project_root_or_cwd
from cos_lib.dispatch_gate import DispatchGate, ProviderCircuitBreaker
...
from cos_lib.deferred_tool_loading import plan_tool_loading, provider_native_defer_payload, record_toolsearch_token_delta, toolsearch_index
...
def _try_registry_provider(
    ...
    if os.environ.get("COS_PROMPT_CACHE", "").strip() == "1":
        try:
            from cos_lib.prompt_cache import maybe_apply_cache
...
def dispatch(
    ...
    for idx, provider in enumerate(providers_requested):
        ...
        if is_fallback and response is not None:
            ...
            try:
                from cos_lib.providers import ADVANCE_ON_RATE_LIMIT_ONLY
        ...
        try:
            from cos_lib.rate_limit_tracker import should_throttle as _rl_should_throttle
        ...
        if response is not None:
            ...
            if _rl_headers and isinstance(_rl_headers, dict):
                try:
                    from cos_lib.rate_limit_tracker import record as _rl_record
