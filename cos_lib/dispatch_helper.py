...
Usage from shell:
    python3 -c "from cos_lib.dispatch_helper import check_slot_availability; \
...
from cos_lib.config_loader import read_top_level_int as _cl_read_top_level_int
from cos_lib.paths import project_root
from cos_lib.tool_result_envelope import wrap_if_large

# ---------------------------------------------------------------------------
# Internal helpers — thin shims that delegate to cos_lib.config_loader
# ---------------------------------------------------------------------------
...
def _find_config_path() -> Optional[str]:
    ...
    Uses :func:`cos_lib.paths.project_root` for env-var resolution (Pattern A),
    then falls back to cwd-relative candidates.  Delegates the full candidate
    logic to :mod:`cos_lib.config_loader`.
    """
...
def _read_max_parallel_agents(config_path: Optional[str] = None) -> int:
    ...
    Delegates to :func:`cos_lib.config_loader.read_top_level_int`.
...
def wrap_dispatch_tool_result(
    ...
    ``cos_lib.agent_runner`` upstream), returns it unchanged (ADR-264 §4).
