...
# Make cos_lib.* importable when invoked directly
_REPO = Path(__file__).resolve().parent.parent
if str(_REPO) not in sys.path:
    sys.path.insert(0, str(_REPO))

from cos_lib.persona_library import (  # noqa: E402
    Persona,
...
def _default_dispatch(prompt: str, model: str) -> dict:
    """Thin wrapper over cos_lib.dispatch.dispatch() returning a plain dict.

    Returns {success, text, cost_usd, provider_used, error}.
    """
    from cos_lib.dispatch import dispatch  # lazy import so tests can stub easily
    result = dispatch(
        prompt=prompt,
...
def run_review(
    ...
    Args:
      ...
      dispatch_fn: test injection point. Real calls go through cos_lib.dispatch.
