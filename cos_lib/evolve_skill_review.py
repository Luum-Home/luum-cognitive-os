...
ADR-262 §Decision 1: reads session turn logs, calls the LLM via cos_lib.dispatch,
...
class EvolveSkillReview:
    ...
    def _dispatch(self, prompt: str) -> str:
        """Call the LLM via cos_lib.dispatch, returning the response text."""
        if self._dispatch_fn is not None:
            return self._dispatch_fn(prompt)

        from cos_lib.dispatch import dispatch  # lazy import — tests can stub via _dispatch_fn
    ...
    def run(self, session_dir: Path | None = None) -> int:
        ...
        for item in candidates:
            ...
            from cos_lib.evolve_task_queue import EvolveProposal
