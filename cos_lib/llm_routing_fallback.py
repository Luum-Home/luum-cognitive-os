...
etc). All LLM calls flow through :func:`cos_lib.dispatch.dispatch`
...
def _project_root() -> Path:
    ...
    try:
        from cos_lib.paths import runtime_project_root_or_cwd  # noqa: WPS433
...
def llm_route(
    ...
    *,
    ...
    ``dispatch_fn`` is a test hook — when omitted, ``cos_lib.dispatch.dispatch``
    is loaded lazily. The function MUST accept the same kwargs as
    :func:`cos_lib.dispatch.dispatch` and return an object exposing
    ...
    if dispatch_fn is None:
        try:
            from cos_lib.dispatch import dispatch as _real_dispatch  # noqa: WPS433
