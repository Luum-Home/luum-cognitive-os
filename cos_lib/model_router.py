...
from cos_lib.execution_profile import resolve_execution_profile
...
def is_advisor_available() -> bool:
    ...
    """
    from cos_lib.anthropic_direct_policy import advisor_strategy_enabled
...
def get_consequence_override(
    ...
    if metrics_dir is None:
        # NOTE: custom resolution — differs from cos_lib.paths.project_root() (Pattern A').
    ...
    if latest_degrade_ts is not None:
        if latest_promote_ts is None or latest_degrade_ts > latest_promote_ts:
            ...
            from cos_lib.dispatch_model_advisor import classify_task_type, _TASK_MODEL_MAP
...
def route_and_execute(
    ...
    """
    from cos_lib.gateway_selector import invalidate_health_cache, select_gateway_for_profile
    ...
    if gateway.name == "litellm":
        from cos_lib.litellm_client import (
            LiteLLMClient,
            LiteLLMError,
            LiteLLMUnavailable,
        )
