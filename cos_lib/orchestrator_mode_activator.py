...
class AutoExecutor:
    ...
    def get_launch_function() -> Optional[Callable]:
        ...
        CONNECTED  → delegate_task from cos_lib.orchestrator_mode
        FIRE_AND_FORGET → None (caller should use Agent tool)
        """
        if not AutoExecutor.should_use_executor():
            return None
        try:
            from cos_lib.orchestrator_mode import delegate_task  # type: ignore[import]
    ...
    def format_launch_advice() -> str:
        ...
        if AutoExecutor.should_use_executor():
            return (
                "CONNECTED: Use delegate_task() from cos_lib.orchestrator_mode "
