...
spawning the engram CLI. The default backend wraps :func:`cos_lib.safe_engram.safe_save`
...
class EngramStore:
    ...
    def save(self, card: Dict[str, Any]) -> None:
        """Persist *card* via :func:`cos_lib.safe_engram.safe_save` (upsert)."""
        # Local import — keeps cos_lib.peer_card importable in environments
...
# ---------------------------------------------------------------------------
# CLI — `python3 -m cos_lib.peer_card <op> [...]`
# ---------------------------------------------------------------------------
