...
Thin wrapper around cos_lib.context_injector that exposes a structured list API
instead of a pre-formatted string.  Designed for callers that want to post-
process or re-rank the matches before formatting.

Usage::

    from cos_lib.query_tailored_context import select_context
...
def select_context(
    ...
    from cos_lib.context_injector import (
        _tokenise,
