...
The COS skill router (`cos_lib/skill_router.py`) routes user prompts across the
...
`python3 -m cos_lib.routing_benchmark`.
"""
...
def write_report_markdown(report: BenchmarkReport, path: Path) -> None:
    ...
    lines.append("")
    lines.append(
        "_Generated by `cos_lib/routing_benchmark.py` (ADR-298). "
...
def regenerate_corpus(
    ...
    *,
    ...
    tests; defaults to ``cos_lib.dispatch.dispatch``. Output is deterministic:
    ...
    if dispatch_fn is None:
        from cos_lib.dispatch import dispatch as _dispatch  # type: ignore
    ...
    header = (
        "# SCOPE: os-only\n"
        "# AUTO-GENERATED via cos_lib.routing_benchmark.regenerate_corpus.\n"
