...
1. [`docs/adrs/INDEX.md`](../adrs/INDEX.md) — full status table for 280 ADRs (sorted: Active, Proposed, Exploration, Resolved, Superseded, Tombstone)
2. [`docs/adrs/STATUS-TAXONOMY.md`](../adrs/STATUS-TAXONOMY.md) — canonical status values + when to use each
3. [`docs/adrs/README.md`](../adrs/README.md) — naming convention, frontmatter schema

## Conventions

- Canonical location: **`docs/adrs/`** only. The legacy `docs/architecture/adrs/` namespace was removed on 2026-05-12 (see ADR-087).
...
- **Write a new ADR**: copy structure from the most recent `docs/adrs/ADR-NNN-*.md`, increment NNN, fill frontmatter, add to relationship graph if it supersedes/extends another.
...
- **Migrate a status**: see [`docs/reports/adr-status-triage-2026-05-12.md`](../reports/adr-status-triage-2026-05-12.md) for the 2026-05-12 normalization pass.
...
- [ADR-087 ADR namespace consolidation](../adrs/ADR-087-adr-namespace-consolidation.md) — why ADRs live in `docs/adrs/`

## Tooling

- `scripts/audit_adrs.py` — frontmatter validator + relationship-chain audit
- `scripts/generate_adr_index.py` — regenerate `docs/adrs/INDEX.md` from frontmatter
- `hooks/adr-detector.sh` — auto-detects architecturally significant commits and drafts ADRs into `docs/adrs/`
