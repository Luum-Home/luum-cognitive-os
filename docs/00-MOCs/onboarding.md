...
1. **Read first**: [`docs/HOW-TO-USE-COS.md`](../HOW-TO-USE-COS.md) — single-page intro
2. **Then**: [`docs/getting-started/`](../getting-started/) — guided setup
3. **Then**: [`docs/onboarding/`](../onboarding/) — deeper orientation
4. **Reference**: [`docs/architecture.md`](../architecture.md) + [`docs/INDEX.md`](../INDEX.md) (the flat catalogue)

## For AI agents (Claude Code, Cursor, Aider, etc.)

1. **Read first**: [`docs/AGENTS.md`](../AGENTS.md) — task-to-doc routing table, canonical glossary, what NOT to read
...
| **ADR** | Architecture Decision Record (in `docs/adrs/`) |
...
More terms in [`docs/AGENTS.md`](../AGENTS.md).
...
- [`docs/business/`](../business/) — vision and roadmap
- [`docs/business/master-plan-checklist.md`](../business/master-plan-checklist.md) — current high-level plan

## Setup

- [`docs/setup/`](../setup/) — local installation
- [`docs/integrations/`](../integrations/) — integrating with external services (Engram, MCP, etc.)
...
- **Read an ADR**: `docs/adrs/ADR-NNN-slug.md` — start with the INDEX
- **Run tests**: `bash scripts/cos-test focused` (lane-scoped) or `pytest tests/audit/ -x`
- **Write an ADR**: copy structure from the most recent in `docs/adrs/`, increment NNN
- **Add a skill**: see the `skill-creator` skill + [`docs/skills/`](../skills/)
