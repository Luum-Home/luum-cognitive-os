...
| Metric | Count | Notes |
|---|---|---|
| Total hook files on disk (`hooks/*.sh`) | **254** | Flat invocable hook scripts under `hooks/`; recount after pruning `singularity-check.sh` and `session-state-save.sh` (the previous **257** never matched the 256 on disk). |
