...
- **Dispatch patterns**: default cascade `--providers qwen,claude` (Qwen primary, Claude fallback on failure); single-provider pinning; inverted priority `--providers claude,qwen` reserved for when quality is critical; programmatic dispatch via `lib.dispatch.dispatch()`; multi-step tool-use tasks via `lib.qwen_agent_loop.run_agent()` — Phase 1 tool whitelist is limited to Read/Edit/Bash, with Grep/WebFetch arriving in a stated future Phase 2.
