...
- **Method:** Real projection into temp consumer sandboxes via the actual installer (`scripts/cos_init.py`), isolated `import lib.<module>` probes replicating each hook's consumer runtime, plus ONE controlled real hook run. No hook bodies executed except the single documented ground-truth run. Repo not mutated.
...
- **None of the 84 lib-importing hooks import the only two projected modules** (`lib.duplicate_scanner`, `lib.project_paths`) — they all import other `lib.*` modules that are simply not on disk anywhere in a consumer.
...
- **84 hooks** in `hooks/*.sh` contain a `from lib.` / `import lib.` / `-m lib.` statement (measured; the orchestrator's earlier "76" is a close undercount — the delta is hooks whose import appears alongside a `sys.path` setup line).
...
For each of the 84 hooks, `python3 -c "import lib.<module>"` was attempted with `cwd = consumer root` and `PYTHONPATH` bound exactly as the hook sets it (empty when it sets none), under two scenarios:
...
- **Positive control:** the same imports (`lib.confidentiality_scanner`, `lib.peer_card`, `lib.rate_limiter`) succeed with `cwd = repo root` and no PYTHONPATH — confirming the probe measures the projection gap, not a broken probe.
