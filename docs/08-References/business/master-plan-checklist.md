...
- [x] Agent stability claims are grouped under [Agent Flicker Control](../../04-Concepts/architecture/agent-flicker-control.md) with `scripts/cos-agent-flicker-report` as the unified runtime receipt for false-completion, retry-thrashing, drift, no-progress, and coordination controls.
...
- [ ] Run the A/B/C falsification benchmark on at least three task classes, following the [SO-Wide Impact Evaluation Plane](../../04-Concepts/architecture/so-wide-impact-evaluation-plane.md), before promoting any broad full-mesh product claim.
- [ ] Implement the top adoption slices from [Gentle-AI Comparative Audit — 2026-06-15](../../04-Concepts/architecture/gentle-ai-comparative-audit-2026-06-15.md): COS SDD status dispatcher, SDD preflight, strict TDD evidence verification, skill-registry path index, phase gatekeeper, and review workload forecast.
...
- [x] Evaluate and implement the first BYO Harness runtime-lab slices from [BYO Harness Comparison — 2026-05-23](../architecture/byoharness-comparison-2026-05-23.md) before wiring a production COS-native agent loop into ADR-291 or Surface 5.
