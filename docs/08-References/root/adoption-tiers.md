...
| Hooks wired | ~107 (minimal profile) | ~143 (standard profile) | ~159 (paranoid profile) |
...
| Primary cost | ~107 hook fires/turn | ~143 hook fires/turn | ~159 hook fires/turn + engram I/O |
...
(`_hook_count: 107`). Enable by copying that profile into `.claude/settings.json`
...
`templates/security-profiles/standard.json` (`_hook_count: 143`).
...
`templates/security-profiles/paranoid.json` (`_hook_count: 159`).
...
The OS fires hooks on every tool call. At lean tier that is ~107 fires; even
...
| Hook fires per turn | ~107, 0 daemons | ~143, 1 daemon (Engram) | ~159, Engram hot-path |
...
<!-- Generated from fe9a40ba on 2026-05-13T13:06:53Z. Do not edit directly. Run `python3 scripts/render_adoption_tiers.py` to regenerate. -->
