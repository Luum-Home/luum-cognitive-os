...
| Primary cost | ~116 hook fires/turn | ~154 hook fires/turn | ~170 hook fires/turn + engram I/O |
...
(`_hook_count: 116`). Enable by copying that profile into `.claude/settings.json`
...
`templates/security-profiles/standard.json` (`_hook_count: 154`).
...
`templates/security-profiles/paranoid.json` (`_hook_count: 170`).
