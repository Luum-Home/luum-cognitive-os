...
- **Runtime**: `lib/pipeline_executor.py` executes workflow YAMLs, invocable via a skill (`/run-pipeline feature my-feature`) or direct CLI (`python3 -m lib.pipeline_executor --workflow ... --change ...`). State persists to `.cognitive-os/pipeline-state/{change}.json` after every phase, resumable with `--resume`.
