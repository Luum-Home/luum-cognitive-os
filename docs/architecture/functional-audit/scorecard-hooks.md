...
> **Scope**: `hooks/*.sh` (154 files) + `hooks/_lib/*.sh` helpers.
...
Refresh answer: the checkout now has **154 hook files**. The current audit
...
| Metric | Count | Notes |
|---|---|---|
| Total hook files on disk (`hooks/*.sh`) | **154** | Flat invocable hook scripts under `hooks/` |
