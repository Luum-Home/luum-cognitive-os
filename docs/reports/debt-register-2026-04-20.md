...
| D45 | orchestrator-2026-04-20 | production_bug | 0 | **RESOLVED fully** | Rate-limiter compounding retry loop — RESOLVED fully across all 3 remaining gaps. Original: retry cap, backoff, circuit breaker, corruption recovery (lib), hook+drainer wiring. Gap A (drainer-as-executor): `hooks/rate-limit-drain.sh` now `subprocess.run()`s safe bash_command items directly with `safe_to_execute()` allowlist; writes audit to `rate-limit-executed.jsonl`; re-enqueues if rate-limited. Gap B (sidecar-file lookup): new `hooks/rate-limit-precheck.sh` (PreToolUse:Bash, runs BEFORE `rate-limiter.sh`) hashes incoming command, looks up queue by `command_hash`, lifts `retry_count` into `RATE_LIMIT_RETRY_COUNT` env, removes matched entry. flock race fix: `RateLimitQueue.enqueue/dequeue_ready/cancel/recover_if_corrupted` all wrapped in `_queue_file_lock()` (5s timeout, logs to `queue-lock-timeout.jsonl`, falls back rather than deadlocking). `rate-limit-precheck.sh` registered in `settings.json` via `apply-efficiency-profile.sh`. 12 new tests (5 drainer-executor + 4 precheck + 3 flock). | see commit | **RESOLVED fully** |
