...
| H6 | Skill schema convention: adopt `description: "Use when…"` across `skills/*/SKILL.md` | ⏸ deferred | 90 SKILL.md files; requires batch agentic pass, not puntual edits. Sonnet pass estimated 1-2 days. | D §🔍13 |

**Wave 1 progress: 5/6 done, 1 deferred. Closed in commits `e7ed3c6b` + `b5062d0f` on `session/fe687ed8-radar-2026-05-08`.**
...
|---|---|---:|---|---|
| M1 | graphiti bi-temporal schema (`valid_from`/`valid_to`) in `memory_relations` | 🔲 not started | A §🔍2c | Apache-2.0 (schema only) |
| M2 | LightRAG dual-level (entity + topic) retrieval scoring → `engram_lifecycle.py` | 🔲 not started | A §🔍2a | MIT (algorithm port) |
| M3 | HippoRAG personalized PageRank as alternative mode in `engram_graph_walker.py` | 🔲 not started | A §🔍2b | MIT (algorithm port) |
| M4 | `memory_class` enum overlay (`semantic`/`episodic`/`procedural`/`working`); couple `memory_decay` to `working` | 🔲 not started | A §🔍12 | MIT (MIRIX overlay) |
...
| # | Topic | Status | Source | License |
|---|---|---:|---|---|
| W3-1 | `lib/repo_map.py`: graph-rank + tree-sitter + token budget. Replaces static allowlist in `lib/context_diet.py` for codegen-context selection | 🔲 not started | D §🔍9 | Apache-2.0 (Aider port) |
...
| W3-3 | Vendor `lib/msgfmt/testdata/` from agentapi (11-harness golden fixtures) → `lib/harness_adapter/testdata/`. No Go sidecar. | 🔲 not started | C §🔍10 | MIT (testdata only) |

**Independence**: W3-1, W3-2, W3-3 share no files — they can run in parallel once Wave 2 lands.
...
**Last updated**: 2026-05-08 by orchestrator session.
