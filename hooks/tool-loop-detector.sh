...
# Read tool info from stdin (Claude hooks receive JSON on stdin)
INPUT=$(cat)
...
TOOL_INPUT=$(echo "$INPUT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(json.dumps(d.get('tool_input',{}), sort_keys=True)[:200])" 2>/dev/null || echo "{}")
