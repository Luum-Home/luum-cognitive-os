...
// RateLimitProtectionValidator enforces hourly token + agent-launch budgets,
// porting hooks/rate-limit-protection.sh. State is read from the cost-events
...
type RateLimitProtectionValidator struct {
...
// NewRateLimitProtectionValidator constructs the validator. Empty costEventsPath
// resolves to $CLAUDE_PROJECT_DIR/.cognitive-os/metrics/cost-events.jsonl.
func NewRateLimitProtectionValidator(costEventsPath string, hourlyTokens int64, agentsPerHour int) *RateLimitProtectionValidator {
	if costEventsPath == "" {
	...
	return &RateLimitProtectionValidator{
...
func (v *RateLimitProtectionValidator) Name() string { return "rate-limit-protection" }
func (v *RateLimitProtectionValidator) Category() validator.ValidatorCategory {
...
func (v *RateLimitProtectionValidator) Validate(_ context.Context, hookCtx *hook.Context) *validator.Result {
	if v.overrideEnv {
	...
	case pct >= 95:
		return &validator.Result{
			...
			Reference: validator.Reference{
				Code: "COS-RATE-002",
				URL:  "docs/architecture/cos-dispatch/rate-limit-protection.md",
			},
...
func (v *RateLimitProtectionValidator) countLastHour() (int64, int) {
