...
PATH_RE = re.compile(r"(?P<path>(?:docs|scripts|hooks|skills|rules|tests|primitive_coverage|\.github|manifests|lib|packages)/[A-Za-z0-9_./\-{}]+)")
...
def evidence_files(root: Path) -> list[Path]:
    patterns = ("scripts/**/*.py", "hooks/**/*.sh", "skills/**/SKILL.md", "rules/**/*.md", "tests/**/*.py", ".github/workflows/*.yml", ".github/workflows/*.yaml", "primitive_coverage/**/*.py", "primitive_coverage/**/*.yaml", "manifests/**/*.json", "lib/**/*.py")
    out: list[Path] = []
...
def classify(root: Path, item: str, decl: str, corpus: str, known: set[str]) -> tuple[str, float, list[str], str]:
    evidence: list[str] = []
    missing: list[str] = []
    for ref in extract_paths(item):
...
def audit(root: Path) -> list[DocsExecutionRow]:
    ...
    for rel, line, kind, item, decl in extract_items(root):
        inferred, confidence, evidence, action = classify(root, item, decl, corpus, known)
