...
def _all_adr_files() -> list[Path]:
    """Return canonical ADR markdown files, excluding OKF synthesis siblings.

    ADR-<N>.synthesis.md files are compact LLM-retrieval summaries generated by
    the OKF KB rollout; they have their own section contract (Decision / Why /
    Consequences / Status / Key links) and must not be checked against the
    full ADR-067+ section requirements. Those synthesis contracts are enforced
    by scripts/validate_okf.py, not this test.
    """
    return sorted(
        p for p in ADRS_DIR.glob("ADR-*.md") if not p.name.endswith(".synthesis.md")
    )
