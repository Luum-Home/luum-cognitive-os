...
ACC_DIR = REPO_ROOT / "docs" / "07-Capabilities" / "acc"
REPORT = ACC_DIR / "latest.json"
COMPACT = ACC_DIR / "latest-compact.md"
GENERATED_ACC_ARTIFACTS = (
    REPORT,
    COMPACT,
    ACC_DIR / "latest.md",
)
...
def test_repository_acc_pipeline_generates_report() -> None:
    original_artifacts = {path: path.read_text() if path.exists() else None for path in GENERATED_ACC_ARTIFACTS}
    try:
