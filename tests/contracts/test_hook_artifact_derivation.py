"""Prueba pareja de scripts/hook_artifact_derivation.py: se lo corre, no se lo nombra.
...
es la mitad que le toca a hook_artifact_derivation.
...
ARTEFACTO = REPO / "scripts/hook_artifact_derivation.py"
...
def _correr(*extra: str) -> subprocess.CompletedProcess:
    if not ARTEFACTO.is_file():
        pytest.skip("scripts/hook_artifact_derivation.py no esta en el arbol")
