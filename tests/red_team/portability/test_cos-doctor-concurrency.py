# SCOPE: os-only
"""Portability proof for scripts/cos-doctor-concurrency.sh."""
...
ARTIFACT = REPO_ROOT / "scripts/cos-doctor-concurrency.sh"


def test_cos_doctor_concurrency_safe_invocation_from_arbitrary_project_root(tmp_path: Path) -> None:
...
def test_cos_doctor_concurrency_has_passing_scope_contract() -> None:
    result = subprocess.run(
        [
            sys.executable,
