# SCOPE: os-only
"""Portability proof for scripts/cos-doctor-harness.sh."""
...
ARTIFACT = REPO_ROOT / "scripts/cos-doctor-harness.sh"


def test_cos_doctor_harness_safe_invocation_from_arbitrary_project_root(tmp_path: Path) -> None:
...
def test_cos_doctor_harness_has_passing_scope_contract() -> None:
    result = subprocess.run(
        [
            sys.executable,
