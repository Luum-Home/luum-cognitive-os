# SCOPE: os-only
"""Portability proof for scripts/cos-doctor-work-inventory.sh."""
...
ARTIFACT = REPO_ROOT / "scripts/cos-doctor-work-inventory.sh"


def test_cos_doctor_work_inventory_safe_invocation_from_arbitrary_project_root(tmp_path: Path) -> None:
...
def test_cos_doctor_work_inventory_has_passing_scope_contract() -> None:
    result = subprocess.run(
        [
            sys.executable,
