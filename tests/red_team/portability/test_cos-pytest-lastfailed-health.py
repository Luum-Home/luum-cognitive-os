# SCOPE: os-only
"""Portability proof for scripts/cos-pytest-lastfailed-health."""
...
ARTIFACT = ROOT / "scripts" / "cos-pytest-lastfailed-health"
...
def test_wrapper_help_runs_from_arbitrary_cwd(tmp_path: Path) -> None:
    ...
    assert "pytest lastfailed" in proc.stdout


def test_wrapper_delegates_to_python_module() -> None:
    text = ARTIFACT.read_text(encoding="utf-8")
    assert "scripts/cos_pytest_lastfailed_health.py" in text
