# SCOPE: os-only
"""Portability proof for scripts/engram-sync.sh."""
...
ARTIFACT = REPO_ROOT / "scripts/engram-sync.sh"


def test_engram_sync_safe_invocation_from_arbitrary_project_root(tmp_path: Path) -> None:
...
def test_engram_sync_has_passing_scope_contract() -> None:
    result = subprocess.run(
        [
            sys.executable,
