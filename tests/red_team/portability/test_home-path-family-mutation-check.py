# SCOPE: os-only
"""Paired portability proof for scripts/home-path-family-mutation-check.sh.
...
ARTIFACT = REPO_ROOT / 'scripts/home-path-family-mutation-check.sh'
...
def test_runs_from_arbitrary_project_root(tmp_path: Path) -> None:
    """cwd-invariance: the script anchors on BASH_SOURCE, not on the caller's cwd."""
    ...
    assert from_foreign.stdout.strip(), "script produced no output from a foreign cwd"
