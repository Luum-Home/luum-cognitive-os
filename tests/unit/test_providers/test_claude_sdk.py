...
def test_is_configured_false_when_no_api_key(monkeypatch):
    ...
    from cos_lib.providers import claude_sdk
...
def test_is_configured_false_when_sdk_not_installed(monkeypatch):
    ...
    with patch("cos_lib.anthropic_direct_policy.direct_anthropic_api_enabled", return_value=True):
...
def test_model_map_has_all_tiers():
    from cos_lib.providers.claude_sdk import MODEL_MAP
...
def test_estimate_cost_positive_for_known_model():
    from cos_lib.providers.claude_sdk import estimate_cost
